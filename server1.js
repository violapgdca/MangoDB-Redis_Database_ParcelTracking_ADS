...
// Route to update parcel location
app.post('/parcels/:trackingId/update-location', async (req, res) => {
    try {
        const { trackingId } = req.params;
        const { nextLocationId } = req.body;
        const db = await connectToDB();
        const parcelsCollection = db.collection(parcelsCollectionName);

        const parcel = await parcelsCollection.findOne({ trackingNumber: trackingId });
        if (!parcel) {
            res.status(404).json({ error: 'Parcel not found' });
            return;
        }

        const newHistory = parcel.history || [];
        newHistory.push(parcel.currentLocation);

        await parcelsCollection.updateOne(
            { trackingNumber: trackingId },
            {
                $set: { currentLocation: new ObjectId(nextLocationId), history: newHistory }
            }
        );
